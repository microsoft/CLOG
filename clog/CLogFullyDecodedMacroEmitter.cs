                ...
                if (module.ManditoryModule || decodedTraceLine.macro.CLogExportModules.Contains(module.ModuleName.ToUpper()))
                {
                    ...
                    if (!decodedTraceLine.configFile.ModuleUniqueness.IsUnique(module, decodedTraceLine, out existingTraceInfo))
                    {
                        ...
                        else
                        {
                            ...
                            CLogConsoleTrace.TraceLine(CLogConsoleTrace.TraceType.Std, "    2. specify the --overwriteHashCollsions command line argument (good if you're making lots of changes that are all safe)");
                            CLogConsoleTrace.TraceLine(CLogConsoleTrace.TraceType.Std, "");
                            CLogConsoleTrace.TraceLine(CLogConsoleTrace.TraceType.Std, "");
